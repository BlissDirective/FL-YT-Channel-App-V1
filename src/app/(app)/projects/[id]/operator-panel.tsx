      ...
      {live && view.hasAnalytics && (
        <div className="mt-4 space-y-2.5 rounded-xl border border-line bg-canvas/50 p-3">
          <div className="flex flex-wrap items-center justify-between gap-1">
            <p className="text-xs font-semibold uppercase tracking-wide text-muted">Growth · Campaign targets</p>
            <span className="text-[11px] text-muted">
              {Math.round(view.mixShortsPct * 100)}% ad variants · {view.mixReason}
            </span>
          </div>
          <Goal label="Audience" value={view.subs} goal={view.subsGoal} fmt={compact} />
          <Goal label="Demo watch-hours · 365d" value={view.watchHours} goal={view.watchGoal} fmt={(n) => n.toFixed(0)} highlight={view.nearerPath === "watch"} />
          {view.shortsViews > 0 && (
            <Goal label="Ad views · 90d" value={view.shortsViews} goal={view.shortsGoal} fmt={compact} highlight={view.nearerPath === "shorts"} />
          )}

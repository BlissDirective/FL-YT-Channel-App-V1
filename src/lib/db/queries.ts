...
export type OperatorView = {
  ...
  /** Program growth progress — present once analytics has been pulled.
      subs = enrolled learners; watchHours = lesson watch-hours (completion);
      shortsViews = preview / micro-lesson views (enrollment reach). */
...
export async function getOperatorView(projectId: string): Promise<OperatorView> {
  ...
  const empty: OperatorView = {
    ...
    autonomy: "autopilot",
    hasAnalytics: false, subs: 0, subsGoal: LEARNERS_GOAL, watchHours: 0, watchGoal: COMPLETION_HOURS_GOAL,
    shortsViews: 0, shortsGoal: PREVIEW_VIEWS_GOAL, retentionPct: 0,

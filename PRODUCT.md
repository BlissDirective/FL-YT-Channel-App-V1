...
| 6 | Variant fan-out stage | `src/lib/pipeline/variants.ts` | ✅ core done (pure hook-variant planner: one approved script → N variants differing only in beat 0, shared-by-reference body so the asset cache reuses VO/visuals; capped fan-out; `variantAdName` + `changedBeatIdxs` helpers; tests in `tests/variants.test.ts`). ⬜ engine wiring: derive sibling videos via `parent_video_id` like derived Shorts |

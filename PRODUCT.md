...
| 6 | Course structure stage | `src/lib/pipeline/course.ts`, `src/lib/actions/course.ts` | ✅ done — core + engine wiring. `seedCourseFromOutlineAction` seeds one IDEA video per lesson (via `lessonSeedRows`); `parseOutlineText` accepts a pasted outline; `CourseSeeder` surface on the Library page; quiz cards are extracted at the scripting seam (`engine.ts` script insert → `attachQuizCards`) into `scripts.metadata.quizCards`. Tests in `tests/course.test.ts`; action manifest updated |

...
| 6 | Course structure stage | `src/lib/pipeline/course.ts` | ✅ core done (buildCourseTree normalizes a loose outline into module→lesson specs with objectives/formats + spend caps; flattenLessons yields the production order for seeding videos; extractQuizCards mines check-for-understanding beats into quiz cards; tests in `tests/course.test.ts`). ⬜ engine wiring: operator seeds one video per FlatLesson; quiz cards ride video metadata into the publish kit |
